...
class TextModel(ModelBase):
    ...
    def get_tokenizer_path(self):
        for candidate in ["tokenizer.model", "tokenizer.model.v3", "tokenizer.json"]:
            path = self.dir_model / candidate
            if path.exists():
                return path
        raise FileNotFoundError(f"No tokenizer file found in {self.dir_model}")
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    if hasattr(torch, "uint16"):
        _dtype_byteswap_map[torch.uint16] = np.uint16
    if hasattr(torch, "uint32"):
        _dtype_byteswap_map[torch.uint32] = np.uint32
    if hasattr(torch, "uint64"):
        _dtype_byteswap_map[torch.uint64] = np.uint64
    if hasattr(torch, "float8_e4m3fn"):
        _dtype_byteswap_map[torch.float8_e4m3fn] = np.uint8
    if hasattr(torch, "float8_e5m2"):
        _dtype_byteswap_map[torch.float8_e5m2] = np.uint8
    ...
    if hasattr(torch, "float8_e4m3fn"):
        _TORCH_F8_DTYPES["F8_E4M3"] = torch.float8_e4m3fn

    if hasattr(torch, "float8_e5m2"):
        _TORCH_F8_DTYPES["F8_E5M2"] = torch.float8_e5m2
